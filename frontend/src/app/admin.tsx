...
type QuestTemplate = { id: string; title: string; description: string; category: Category; difficulty: Difficulty; radius_km: number | null; is_active: boolean };
...
export default function AdminScreen() {
  ...
  const createTemplate = async () => {
    ...
    try {
      const response = await api.post<QuestTemplate>('/admin/quest-templates', { title: title.trim(), description: description.trim(), category, difficulty, country_code: countryCode, radius_km: category === 'local' ? localRadius : null });
      setTemplates((current) => [response.data, ...current]);
      setTitle(''); setDescription(''); setCategory('daily'); setDifficulty('easy'); setRadius('40'); setCountryCode(null);
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      {loading ? <ActivityIndicator color="#38bdf8" style={styles.loader} /> : error !== '' ? <Text style={styles.error}>{error}</Text> : <>
        {canManageTemplates && <><Text style={styles.intro}>Twórz i zarządzaj pulą challenge’y.</Text><View style={styles.formCard}>
          ...
          <Text style={styles.fieldLabel}>KRAJ (OPCJONALNIE)</Text><CountrySelect value={countryCode} onChange={setCountryCode} />
