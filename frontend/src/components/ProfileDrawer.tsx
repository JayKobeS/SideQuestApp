      ...
      <Animated.View style={[styles.modal, {
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          ...
          <View style={styles.xpCard}>
            ...
            <View style={styles.progressTrack}><View style={[styles.progressBar, { width: `${Math.min(100, profile.xp % 100)}%` }]} /></View>
            <Text style={styles.xpHint}>{100 - (profile.xp % 100)} XP do następnego poziomu</Text>
          </View>
          ...
          {loading ? <ActivityIndicator color="#38bdf8" style={styles.loader} /> : profile.quests.length === 0 ? (
...
function AccountSettings(props: AccountSettingsProps) {
  ...
  return <View style={styles.settingsContent}>
    ...
    {isMenu ? <>
      ...
      {isEmail && <TextInput style={styles.input} value={props.newEmail} onChangeText={props.onNewEmailChange} placeholder="Nowy adres e-mail" placeholderTextColor="#64748b" keyboardType="email-address" autoCapitalize="none" />}
      <TextInput style={styles.input} value={props.currentPassword} onChangeText={props.onCurrentPasswordChange} placeholder="Aktualne hasło" placeholderTextColor="#64748b" secureTextEntry />
      {!isEmail && <><TextInput style={styles.input} value={props.newPassword} onChangeText={props.onNewPasswordChange} placeholder="Nowe hasło (min. 6 znaków)" placeholderTextColor="#64748b" secureTextEntry /><TextInput style={styles.input} value={props.repeatPassword} onChangeText={props.onRepeatPasswordChange} placeholder="Powtórz nowe hasło" placeholderTextColor="#64748b" secureTextEntry /></>}
...
const styles = StyleSheet.create({
  ...
  modal: { maxHeight: '84%', backgroundColor: '#0b1225', borderWidth: 1, borderColor: 'rgba(56, 189, 248, 0.3)', borderRadius: 30, boxShadow: '0px 14px 28px rgba(0, 0, 0, 0.55)', overflow: 'hidden' },
  ...
  closeButton: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(15, 23, 42, 0.82)', borderWidth: 1, borderColor: 'rgba(148, 163, 184, 0.25)' }, closeButtonText: { color: '#e2e8f0', fontSize: 28, lineHeight: 30, fontWeight: '300' },
  content: { padding: 24, paddingBottom: 36 }, sectionLabel: { color: '#7dd3fc', fontSize: 11, fontWeight: '800', letterSpacing: 1.6 },
  identityRow: { flexDirection: 'row', alignItems: 'center', marginTop: 24, marginBottom: 24 }, avatarLarge: { width: 72, height: 72, borderRadius: 36, backgroundColor: '#2563eb', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#7dd3fc', boxShadow: '0px 0px 14px rgba(37, 99, 235, 0.65)' }, avatarLargeText: { color: '#fff', fontSize: 25, fontWeight: '800', letterSpacing: 1 }, identityText: { marginLeft: 15, flex: 1 }, username: { color: '#f8fafc', fontSize: 23, fontWeight: '800' }, email: { color: '#94a3b8', fontSize: 12, marginTop: 3 }, levelBadge: { alignSelf: 'flex-start', marginTop: 8, paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, backgroundColor: 'rgba(56, 189, 248, 0.13)' }, levelText: { color: '#67e8f9', fontSize: 10, fontWeight: '800', letterSpacing: 1 },
  xpCard: { backgroundColor: 'rgba(15, 23, 42, 0.88)', borderWidth: 1, borderColor: 'rgba(56, 189, 248, 0.22)', borderRadius: 16, padding: 16, marginBottom: 28 }, xpHeading: { flexDirection: 'row', justifyContent: 'space-between' }, xpLabel: { color: '#94a3b8', fontSize: 11, fontWeight: '700', letterSpacing: 1 }, xpValue: { color: '#f8fafc', fontSize: 12, fontWeight: '800' }, progressTrack: { height: 7, borderRadius: 4, overflow: 'hidden', backgroundColor: '#1e293b', marginTop: 12 }, progressBar: { height: '100%', minWidth: 4, backgroundColor: '#38bdf8', borderRadius: 4 }, xpHint: { color: '#64748b', fontSize: 11, marginTop: 9 },
  statsRow: { flexDirection: 'row', gap: 10, marginTop: 12 }, stat: { flex: 1, paddingVertical: 15, alignItems: 'center', backgroundColor: 'rgba(15, 23, 42, 0.68)', borderRadius: 14, borderWidth: 1, borderColor: 'rgba(148, 163, 184, 0.12)' }, statValue: { color: '#f8fafc', fontSize: 24, fontWeight: '800' }, statLabel: { color: '#64748b', fontSize: 9, fontWeight: '800', letterSpacing: 0.8, marginTop: 4 },
  missionsLabel: { marginTop: 28 }, loader: { marginVertical: 32 }, emptyState: { paddingVertical: 27, paddingHorizontal: 18, alignItems: 'center', backgroundColor: 'rgba(15, 23, 42, 0.48)', borderRadius: 16, marginTop: 12 }, emptyIcon: { color: '#38bdf8', fontSize: 24, marginBottom: 6 }, emptyTitle: { color: '#e2e8f0', fontSize: 15, fontWeight: '700' }, emptyDescription: { color: '#64748b', textAlign: 'center', fontSize: 12, lineHeight: 18, marginTop: 6 },
  questRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 13, borderBottomWidth: 1, borderColor: 'rgba(148, 163, 184, 0.1)' }, questDot: { width: 9, height: 9, borderRadius: 5, backgroundColor: '#38bdf8', marginRight: 11 }, questDotDone: { backgroundColor: '#34d399' }, questInfo: { flex: 1 }, questTitle: { color: '#e2e8f0', fontSize: 13, fontWeight: '700' }, questCountry: { color: '#64748b', fontSize: 11, marginTop: 2 }, questStatus: { color: '#7dd3fc', fontSize: 9, fontWeight: '800', letterSpacing: 0.6 }, questStatusDone: { color: '#6ee7b7' },
  settingsButton: { minHeight: 52, marginTop: 28, paddingHorizontal: 16, borderWidth: 1, borderColor: 'rgba(148, 163, 184, 0.24)', borderRadius: 13, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, settingsButtonText: { color: '#cbd5e1', fontSize: 11, fontWeight: '800', letterSpacing: 0.8 }, arrow: { color: '#7dd3fc', fontSize: 23 }, logoutButton: { alignItems: 'center', paddingVertical: 18, marginTop: 7 }, logoutButtonText: { color: '#fca5a5', fontSize: 11, fontWeight: '800', letterSpacing: 1.1 },
  ...
  settingsContent: { minHeight: 410, paddingTop: 4 }, backButton: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', marginBottom: 26 }, backArrow: { color: '#7dd3fc', fontSize: 28, lineHeight: 22, marginRight: 7 }, backText: { color: '#7dd3fc', fontSize: 11, fontWeight: '800', letterSpacing: 1 }, settingsTitle: { color: '#f8fafc', fontSize: 25, fontWeight: '800' }, settingsDescription: { color: '#94a3b8', fontSize: 13, lineHeight: 19, marginTop: 7, marginBottom: 25 }, settingOption: { minHeight: 76, marginBottom: 12, paddingHorizontal: 17, borderWidth: 1, borderColor: 'rgba(148, 163, 184, 0.2)', borderRadius: 15, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: 'rgba(15, 23, 42, 0.55)' }, settingOptionTitle: { color: '#e2e8f0', fontSize: 14, fontWeight: '700' }, settingOptionSub: { color: '#64748b', fontSize: 11, marginTop: 5 }, input: { height: 52, paddingHorizontal: 15, marginBottom: 12, borderRadius: 13, borderWidth: 1, borderColor: 'rgba(148, 163, 184, 0.3)', backgroundColor: 'rgba(2, 6, 23, 0.48)', color: '#f8fafc', fontSize: 14 }, feedback: { marginBottom: 13, fontSize: 12, lineHeight: 18 }, feedbackError: { color: '#fca5a5' }, feedbackSuccess: { color: '#6ee7b7' }, saveButton: { minHeight: 52, marginTop: 8, borderRadius: 14, backgroundColor: '#2563eb', borderWidth: 1, borderColor: '#60a5fa', alignItems: 'center', justifyContent: 'center', boxShadow: '0px 0px 10px rgba(37, 99, 235, 0.4)' }, saveButtonDisabled: { opacity: 0.65 }, saveButtonText: { color: '#fff', fontSize: 11, fontWeight: '800', letterSpacing: 1 },
});
